...
class TagsTable2 extends React.Component {
  constructor(props) {
    super(props);
  ...
  getMetaArray(instanceNumber) {
    if (
      typeof this.state.instanceNumbers !== "undefined" &&
    ...
    if (this.isDicomMeta(this.state.fullMetaData)) {
      reducer = this.getDicomTagReducer(
  ...
  render() {
    const { classes, MyFunction } = this.props;
    const { displayData, searchfor, sliderMin, sliderMax } = this.state;

    MyFunction(displayData);

    return <></>;
  }
}

TagsTable2.propTypes = {
  classes: PropTypes.object.isRequired,
};

export default withStyles(styles)(TagsTable2);
